...
function AnalyticsContent() {
  ...
  const { data: analyticsResponse, isLoading: loadingAnalytics, refetch: refetchAnalytics } = api.analytics.overview.useQuery(
    {
  ...
  const { data: deviceData = [], isLoading: loadingDevices, refetch: refetchDevices } = api.analytics.devices.useQuery(
    {
      websiteId: activeWebsiteId,
      startDate: startDate,
      endDate: endDate,
  ...
  const { data: topPages = [], isLoading: loadingPages, refetch: refetchPages } = api.analytics.pages.useQuery(
    {
      websiteId: activeWebsiteId,
      startDate: startDate,
      endDate: endDate,
            ...
            {[
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (tab.key === "overview") url.searchParams.delete("tab");
                  else url.searchParams.set("tab", tab.key);
...
function FunnelsTab({ websiteId }: { websiteId: string }) {
  ...
  const { data: funnelsList = [], isLoading } = api.funnels.list.useQuery({ websiteId });
  const { data: suggestions } = api.funnels.suggestions.useQuery({ websiteId });

  const createFunnel = api.funnels.create.useMutation({
    onSuccess() {
  ...
  const deleteFunnel = api.funnels.delete.useMutation({
    onSuccess() {
      utils.funnels.list.invalidate();
    },
  });
  ...
  const { data: analysis } = api.funnels.analyze.useQuery(
    { id: selectedFunnel! },
