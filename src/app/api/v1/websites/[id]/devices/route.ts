...
import { websites, visitors } from "@/server/db/schema"
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const dateFilter = and(eq(visitors.websiteId, id), gte(visitors.createdAt, start), lte(visitors.createdAt, end))

  const [deviceBreakdown, browserBreakdown, osBreakdown] = await Promise.all([
    db
      .select({ device: visitors.device, count: count() })
      .from(visitors)
      .where(dateFilter)
      ...
      .limit(20),
    db
      .select({ browser: visitors.browser, count: count() })
      .from(visitors)
      .where(dateFilter)
      ...
      .limit(20),
    db
      .select({ os: visitors.os, count: count() })
      .from(visitors)
      .where(dateFilter)
      .groupBy(visitors.os)
      .orderBy(desc(count()))
