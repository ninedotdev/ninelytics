...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referrers = await db
    .select({
      referrerDomain: visitorSessions.referrerDomain,
    ...
    .groupBy(visitorSessions.referrerDomain, visitorSessions.utmSource, visitorSessions.utmMedium, visitorSessions.utmCampaign)
    .orderBy(desc(count()))
    .limit(20)
