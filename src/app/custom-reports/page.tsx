...
export default function CustomReportsPage() {
  ...
  const { data: reportsData = [], isLoading: loadingReports, refetch: refetchReports } = api.customReports.list.useQuery(
    {
  ...
  const createReport = api.customReports.create.useMutation({
    onSuccess: () => {
  ...
  const deleteReport = api.customReports.delete.useMutation({
    onSuccess: () => {
  ...
  const handleCreateReport = async () => {
    ...
    try {
      // Prepare data, converting "NONE" schedule to undefined
      const dataToSend = {
        ...formData,
  ...
  const handleExecuteReport = (report: CustomReport) => {
    // Navigate to a results page or open a modal with results
  ...
  // Filter reports by selected website
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {group.metrics.map((metric) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`p-2 rounded ${metric.color} bg-opacity-10`}
                              >
                                <Icon size={16} />
                              </div>
                ...
                <div className="space-y-2">
                  ...
                  {formData.schedule && formData.schedule !== "NONE" && (
                    <p className="text-xs text-gray-500">
                      Future feature: Reports will be automatically generated
                      and emailed
                    </p>
                  )}
                </div>
