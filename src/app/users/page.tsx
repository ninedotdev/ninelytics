...
export default function UsersPage() {
  ...
  const [bulkNewRole, setBulkNewRole] = useState<"ADMIN" | "OWNER" | "VIEWER">(
    "VIEWER"
  );
  ...
  const { data: usersData = [], isLoading: loadingUsers, refetch: refetchUsers } = api.users.list.useQuery({
          ...
          <Card>
            <CardContent className="p-0">
              <div className="divide-y">
                <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-muted/50">
                  {[1, 4, 2, 2, 2, 1].map((span, i) => (
                    <div key={`skel-${span}-${i}`} className={`col-span-${span} flex items-center`}>
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="grid grid-cols-12 gap-4 px-6 py-4 items-center">
                    ...
                    <div className="col-span-1 flex justify-end"><Skeleton className="h-8 w-8" /></div>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          >
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Send an invitation to a new team member to join your analytics
                  dashboard.
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    onValueChange={(value) =>
                      setValue("role", value as "ADMIN" | "OWNER" | "VIEWER")
                    }
                  >
                    <SelectTrigger>
              ...
              <div className="divide-y">
                ...
                {filteredUsers.map((user) => (
                  ...
                  >
                    ...
                    <Badge variant={getRoleBadgeVariant(user.role)} className="shrink-0 text-[10px]">
                      {user.role}
                    </Badge>

                    <span className="hidden md:inline text-xs text-muted-foreground w-24 shrink-0">
                      {user._count.ownedWebsites} sites
                    </span>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete {selectedUsers.size} user(s). This
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select
                value={bulkNewRole}
                onValueChange={(v) =>
                  setBulkNewRole(v as "ADMIN" | "OWNER" | "VIEWER")
                }
              >
                <SelectTrigger>
                  <SelectValue />
