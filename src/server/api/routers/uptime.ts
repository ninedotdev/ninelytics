...
import { websites, users, uptimeChecks, uptimeIncidents } from "@/server/db/schema"
...
export const uptimeRouter = router({
  ...
  updateSettings: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Start scheduler if enabling for the first time
      if (input.uptimeEnabled) {
        try {
          const { start } = await import("workflow/api")
          const { uptimeScheduler } = await import("@/workflows/uptime-monitor")
          await start(uptimeScheduler, [])
        } catch {
          // Scheduler may already be running — that's fine
        }
      }
  ...
  triggerCheck: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await start(uptimeCheckSingle, [input.websiteId])
