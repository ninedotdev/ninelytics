...
import { funnels, funnelSteps, pageViews, events, websites, userWebsiteAccess } from "@/server/db/schema"
import { eq, and, or, sql, desc, asc } from "drizzle-orm"

const ensureWebsiteAccess = async (db: typeof import("@/server/db/client").db, websiteId: string, userId: string) => {
  const rows = await db
    .select({
      id: websites.id,
      name: websites.name,
...
const ensureFunnelAccess = async (db: typeof import("@/server/db/client").db, funnelId: string, userId: string) => {
  const rows = await db
    .select({
      funnel: funnels,
