...
export const Route = createFileRoute("/_app/speed-insights")({
  component: SpeedInsightsPage,
});
...
function SpeedInsightsPage() {
  ...
  const { data: websitesData } = trpc.websites.optimized.useQuery({ page: 1, pageSize: 100 });
  ...
  const { data: statusData } = trpc.speedInsights.getStatus.useQuery(
    effectiveWebsite ? { websiteId: effectiveWebsite } : skipToken
  );
  const isEnabled = statusData?.enabled ?? false;

  const { data: summary, isLoading: summaryLoading } = trpc.speedInsights.getSummary.useQuery(
  ...
  const { data: timeSeries, isLoading: timeSeriesLoading } = trpc.speedInsights.getTimeSeries.useQuery(
    effectiveWebsite && isEnabled
  ...
  const { data: worstPages, isLoading: worstPagesLoading } = trpc.speedInsights.getWorstPages.useQuery(
    effectiveWebsite && isEnabled
      ? { websiteId: effectiveWebsite, vitalName: selectedVital, period }
      : skipToken,
    { refetchInterval: 30_000 }
  ...
  const toggle = trpc.speedInsights.toggle.useMutation({
            ...
            <CardContent className="p-6 flex items-center gap-6">
              <div className="flex-shrink-0 relative w-24 h-24">
                <svg viewBox="0 0 36 36" className="w-24 h-24 -rotate-90">
                  <circle cx="18" cy="18" r="15.9" fill="none" strokeWidth="2.5" className="stroke-muted/40" />
