...
import { IconTrash, IconCopy, IconEye, IconEyeOff, IconBrandGoogle } from '@tabler/icons-react'
...
function SettingsPage() {
  ...
  const { data: apiKeysData = [], refetch: refetchApiKeys } = trpc.settings.getApiKeys.useQuery()
  ...
  const { data: tgPrefs, refetch: refetchTgPrefs } = trpc.uptime.getNotificationPrefs.useQuery()
  const pairTelegram = trpc.uptime.pairTelegram.useMutation({
    onSuccess(data) {
  ...
  const unpairTelegram = trpc.uptime.unpairTelegram.useMutation({
    onSuccess() {
  ...
  const { data: cfZones, refetch: refetchCfZones } = trpc.cloudflare.listZones.useQuery()

  const saveCfToken = trpc.cloudflare.saveToken.useMutation({
    onSuccess(result) {
  ...
  const removeCfToken = trpc.cloudflare.removeToken.useMutation({
    onSuccess() {
  ...
  const { data: googleStatus, refetch: refetchGoogleStatus } = trpc.googleAnalytics.getConnectionStatus.useQuery()
  const { data: gaProperties, refetch: refetchGaProperties } = trpc.googleAnalytics.listProperties.useQuery()
  const { data: scSites } = trpc.searchConsole.listSites.useQuery()

  const disconnectGoogle = trpc.googleAnalytics.disconnect.useMutation({
    onSuccess() {
  ...
  const removeGaCreds = trpc.googleAnalytics.removeCredentials.useMutation({
    onSuccess() {
              ...
              <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="flex-1 flex flex-col space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input id="currentPassword" type={showCurrentPw ? "text" : "password"} {...passwordForm.register('currentPassword')} />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input id="newPassword" type={showNewPw ? "text" : "password"} {...passwordForm.register('newPassword')} />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input id="confirmPassword" type={showConfirmPw ? "text" : "password"} {...passwordForm.register('confirmPassword')} />
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Connect your Cloudflare account to import historical analytics</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {cfZones?.hasToken ? (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">Go to any website&apos;s settings to link a zone and sync data.</p>
                  <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 w-full"
                    onClick={() => removeCfToken.mutate()} disabled={removeCfToken.isPending}>
                    Disconnect
                  </Button>
                </>
              ) : (
                <>
                  <div className="space-y-2">
                    ...
                    <Input id="cfToken" type="password" value={cfToken} onChange={(e) => setCfToken(e.target.value)} placeholder="Your Cloudflare API token" />
                    <p className="text-xs text-muted-foreground">Create a token with Zone:Read + Analytics:Read permissions.</p>
                  </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Connect your Google account for Analytics and Search Console access</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {googleStatus?.hasOAuth ? (
                <>
                  <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <div className="h-2 w-2 rounded-full bg-green-500" />
                    <span className="text-sm font-medium text-green-800 dark:text-green-200">Connected via OAuth</span>
                  </div>
                  ...
                  <p className="text-xs text-muted-foreground">Go to any website&apos;s settings to link GA4 properties and Search Console sites.</p>
                  <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 w-full"
                    onClick={() => disconnectGoogle.mutate()} disabled={disconnectGoogle.isPending}>
                    Disconnect Google
                  </Button>
                </>
                ...
                <>
                  <div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                    ...
                    <span className="text-sm font-medium text-yellow-800 dark:text-yellow-200">Service Account (legacy)</span>
                  ...
                  <p className="text-xs text-muted-foreground">Upgrade to OAuth to access Search Console and simplify your setup.</p>
                  <div className="flex gap-2">
                    <Button className="flex-1" onClick={() => { window.location.href = '/api/google/auth' }}>
                      <IconBrandGoogle size={16} className="mr-2" />
                      Upgrade to OAuth
                    </Button>
                    <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700"
                      onClick={() => removeGaCreds.mutate()} disabled={removeGaCreds.isPending}>
                      Remove
                    </Button>
                  </div>
                </>
                ...
                <>
                  <p className="text-sm text-muted-foreground">Connect your Google account to import GA4 analytics and Search Console data.</p>
                  <Button className="w-full" onClick={() => { window.location.href = '/api/google/auth' }}>
                    <IconBrandGoogle size={16} className="mr-2" />
                    Connect with Google
                  </Button>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Receive uptime alerts via Telegram bot</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {tgPrefs?.telegramChatId ? (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">Uptime alerts will be sent to this Telegram chat.</p>
                  <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 w-full"
                    onClick={() => unpairTelegram.mutate()} disabled={unpairTelegram.isPending}>
                    Disconnect
                  </Button>
                </>
              ) : (
                <>
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="tgBotToken" type="password" value={tgBotToken} onChange={(e) => setTgBotToken(e.target.value)} placeholder="123456:ABC-DEF..." className="font-mono text-xs" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="tgChatId" className="text-xs">Chat ID</Label>
                      <Input id="tgChatId" value={tgChatId} onChange={(e) => setTgChatId(e.target.value)} placeholder="123456789" className="font-mono text-xs" />
                    </div>
                  </div>
                  <Button className="w-full"
