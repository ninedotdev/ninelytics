...
function AdminPage() {
  ...
  useEffect(() => {
    if (!sessionLoading && session && !session.user?.isSuperAdmin) {
  ...
  const { data: health } = trpc.admin.system.health.useQuery(
    undefined,
    { enabled: session?.user?.isSuperAdmin === true }
  );
  ...
  const refreshViews = trpc.admin.system.refreshViews.useMutation({
    onSuccess() {
  ...
  const clearCache = trpc.admin.cache.clear.useMutation({
    onSuccess() {
  ...
  const clearQueue = trpc.admin.eventQueue.clear.useMutation({
    onSuccess() {
  ...
  const isPersonalMode = import.meta.env.VITE_IS_MULTI_TENANT !== "true";
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-lg border p-3">
                ...
                <p className="text-sm font-medium flex items-center gap-2">
                  <span className={`h-2 w-2 rounded-full inline-block ${health.database.connected ? "bg-green-500" : "bg-red-500"}`} />
