...
export const Route = createFileRoute("/_app/custom-reports/$id/view")({
...
function ViewCustomReportPage() {
  ...
  const { data: reportData_query, isLoading: loadingReport, error: reportError } = trpc.customReports.byId.useQuery(
    { id }
  );
  ...
  useEffect(() => {
    if (reportError) {
      ...
      router.navigate({ to: "/custom-reports" });
    }
  }, [reportError, router]);
  ...
  const { data: executeData, isLoading: loadingExecute, refetch: refetchExecute } = trpc.customReports.execute.useQuery(
    {
      id,
  ...
  const { data: analyticsOverview, isLoading: loadingAnalytics } = trpc.analytics.overview.useQuery(
    {
      websiteId: websiteId || "",
      startDate: startDate.toISOString().split("T")[0],
      endDate: endDate.toISOString().split("T")[0],
