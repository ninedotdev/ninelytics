...
function parseSegments(text: string): Segment[] {
  ...
  // Tolerant regex: case-insensitive, allows the model to wrap the JSON
  // in ```json ... ``` code fences, and accepts any whitespace around the
  // tags. The group is non-greedy so multiple charts parse independently.
  const regex = /\[CHART_DATA\]\s*([\s\S]*?)\s*\[\/CHART_DATA\]/gi;
  let lastIndex = 0;
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    try {
      ...
      if (chart.type && Array.isArray(chart.data) && chart.data.length > 0) {
        ...
        console.warn("[ai-chat] chart missing required fields", chart);
        segments.push({ type: "text", content: match[0]! });
      }
    } catch (err) {
      console.warn("[ai-chat] chart JSON parse failed:", err, jsonStr);
      segments.push({ type: "text", content: match[0]! });
    }
...
function MessageParts({ message, isLastMessage, isStreaming }: { message: UIMessage; isLastMessage: boolean; isStreaming: boolean }) {
  ...
  // If the text has [CHART_DATA] but parseSegments couldn't extract a
  // valid chart (bad JSON etc.), fall back to segment rendering anyway
  // so the raw tags aren't shown as a literal markdown block.
  const hasAttempt = /\[CHART_DATA\]/i.test(fullText);
  ...
  return (
    <>
      ...
      {pending ? (
        ...
        // min-w forces the MessageContent's w-fit to expand so charts
        // have room to render. 100% of the content column = chart
        // actually readable instead of collapsed to title width.
        <div className="space-y-3 w-full min-w-[min(560px,100%)]">
          {segments.map((seg, j) =>
            seg.type === "text" ? (
              ...
              <div
                key={`${message.id}-${j}`}
                className="rounded-xl border bg-card p-3 overflow-hidden w-full"
              >
...
function AIAnalyticsPage() {
  ...
  const { data: insightsData, isLoading: loadingInsights } = trpc.ai.insights.useQuery(
    { websiteId: selectedWebsite, timeRange: "30" },
    { enabled: !!selectedWebsite }
  );
  const { data: anomaliesData = [], isLoading: loadingAnomalies } = trpc.ai.anomalies.useQuery(
    { websiteId: selectedWebsite },
    { enabled: !!selectedWebsite }
  );
  const { data: predictionsData, isLoading: loadingPredictions } = trpc.ai.predictions.useQuery(
    { websiteId: selectedWebsite, days: 7 },
    { enabled: !!selectedWebsite }
  );
  const { data: recommendationsData = [], isLoading: loadingRecommendations } = trpc.ai.recommendations.useQuery(
    { websiteId: selectedWebsite },
    { enabled: !!selectedWebsite }
  );
  const { data: stripeRevenue } = trpc.stripe.revenue.useQuery(
  ...
  const { data: speedData } = trpc.speedInsights.getSummary.useQuery(
    { websiteId: selectedWebsite, period: "30d" },
    { enabled: !!selectedWebsite }
  );
  const { data: scData } = trpc.searchConsole.getSummary.useQuery(
    { websiteId: selectedWebsite, days: 30 },
    { enabled: !!selectedWebsite }
  );
  const { data: sitemapData } = trpc.sitemap.getSettings.useQuery(
    { websiteId: selectedWebsite },
    { enabled: !!selectedWebsite }
  );
